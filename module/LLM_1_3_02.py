...
def get_simple_comfort_message(analysis_data):
    """生成简单的安抚消息"""
    emotion = analysis_data['mark_a']['primary_emotion']
    intensity = analysis_data['mark_b']['intensity']

    comfort_messages = {
        "悲伤": "我能感受到您现在的情绪，请记住您并不孤单。每个人都会有情绪波动的时候，这是完全正常的。",
        "愤怒": "我理解您现在可能有些生气，让我们先深呼吸几次。强烈的情绪需要被看见和理解。",
        "恐惧": "害怕是正常的反应，您现在很安全。让我们一起面对这些感受。",
        "厌恶": "我理解您的反感情绪，有些情况确实让人难以接受。",
        "高兴": "很高兴看到您有积极的情绪！请享受这份美好。",
        "惊讶": "意外的情况确实会让人感到惊讶，我理解您的感受。"
    }

    message = comfort_messages.get(emotion, "我理解您现在的感受，情绪是我们内心的重要信号。")

    # 如果是紧急情况，添加热线建议
    if analysis_data['safety_result']['final_safety_level'] == 'critical':
        message += " 如果您需要进一步帮助，建议拨打以下心理健康热线。"
...
def safety_classification(mark_a, mark_b, text):
    ...
    # 如果是critical级别，设置中断标志（但实际不会中断，只是标记）
...
def analyze_user_text(text):
    ...
    # 5. 生成简单的安抚消息
    comfort_message = get_simple_comfort_message(analysis_data)
    print(f"💬 安抚消息: {comfort_message}")

    # 6. 如果达到critical级别，提供热线建议但不中断
    if safety_result['should_interrupt']:
        print("🚨 检测到紧急情况，建议寻求专业帮助...")
        ...
        print("💡 建议拨打以上热线寻求专业帮助！")

        # 返回结果但不中断
        result = {
            "processing_complete": True,  # 改为True，继续后续处理
            ...
            "message": "检测到紧急情况，已提供安抚消息和心理健康热线建议，但继续后续处理"
        }

        print("⚠️ 检测到紧急情况，但继续后续处理")
...
def main():
    ...
    while True:
        ...
        try:
            ...
            print(f"   安抚消息: {result.get('comfort_message', '')}")

            if result.get('emergency_triggered'):
                print("   🚨 检测到紧急情况，建议拨打心理健康热线！")
